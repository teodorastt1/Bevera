...
namespace Bevera.Controllers
{
    ...
    public class AdminRolesController : Controller
    {
        ...
        public async Task<IActionResult> Edit(EditUserRoleViewModel vm)
        {
            ...
            await SyncDistributorProfileAsync(user, currentRole, vm.RoleName);

            TempData["FlashMessage"] = vm.RoleName == "Distributor"
                ? "Ролята е променена. Потребителят вече е активен дистрибутор."
                : "Ролята е променена.";
        ...
        {
            if (string.Equals(vm.RoleName, "Distributor", StringComparison.OrdinalIgnoreCase))
            {
                ModelState.AddModelError(nameof(vm.RoleName), "Админът няма право да създава дистрибутори директно. Потребителят трябва сам да се регистрира, а после да му смениш ролята.");
            }
        ...
        private async Task SyncDistributorProfileAsync(ApplicationUser user, string? oldRole, string? newRole)
        {
            var existingDistributor = await _db.Distributors.FirstOrDefaultAsync(x => x.ApplicationUserId == user.Id || (x.Email != null && x.Email == user.Email));

            if (string.Equals(newRole, "Distributor", StringComparison.OrdinalIgnoreCase))
            {
                var displayName = $"{user.FirstName} {user.LastName}".Trim();
                if (string.IsNullOrWhiteSpace(displayName))
                    displayName = user.Email ?? "Дистрибутор";

                if (existingDistributor == null)
                {
                    existingDistributor = new Models.Supply.Distributor
                    {
                        Name = displayName,
                        Email = user.Email,
                        Phone = user.PhoneNumber,
                        Address = user.Address,
                        Notes = "Профилът е създаден автоматично след смяна на роля от админ.",
                        IsActive = true,
                        CreatedAt = DateTime.UtcNow,
                        ApplicationUserId = user.Id
                    };

                    _db.Distributors.Add(existingDistributor);
                }
                else
                {
                    existingDistributor.Name = string.IsNullOrWhiteSpace(existingDistributor.Name) ? displayName : existingDistributor.Name;
                    existingDistributor.Email = user.Email;
                    existingDistributor.Phone = user.PhoneNumber;
                    existingDistributor.Address = string.IsNullOrWhiteSpace(existingDistributor.Address) ? user.Address : existingDistributor.Address;
                    existingDistributor.ApplicationUserId = user.Id;
                    existingDistributor.IsActive = true;
                }
            ...
            if (string.Equals(oldRole, "Distributor", StringComparison.OrdinalIgnoreCase) && existingDistributor != null)
            {
                existingDistributor.IsActive = false;
                await _db.SaveChangesAsync();
            }
        }
    }
