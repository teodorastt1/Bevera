...
namespace Bevera.Data
{
    public static class IdentitySeed
    {
        public static async Task SeedAsync(IServiceProvider services)
        {
            ...
            var distributorUser = await userManager.FindByEmailAsync("distributor@bevera.local");
            if (distributorUser != null && !db.Distributors.Any(x => x.Email == distributorUser.Email))
            {
                db.Distributors.Add(new Models.Supply.Distributor
                {
                    Name = "Demo Distributor",
                    Email = distributorUser.Email,
                    Phone = "+359 000 000 000",
                    Address = "Разград",
                    Notes = "Демо дистрибутор за симулация на отделен dashboard.",
                    IsActive = true,
                    CreatedAt = DateTime.UtcNow,
                    ApplicationUserId = distributorUser.Id
                });
